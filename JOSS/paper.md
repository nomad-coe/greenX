...
\noindent\begin{minipage}{.47\textwidth}
  \fontsize{8}{10}\selectfont
  \begin{align}
    F^\text{even}(i\omega_k) &= \sum_{j=1}^{n} \delta_{kj} \mathrm{cos}(\omega_k\tau_j)\hat{F}^\text{even}(i\tau_j)\hspace{1.5em}{\color{white}.}\label{ct_st_even}
    \\
    \hat{F}^\text{even}(i\tau_j) &= \sum_{k=1}^{n} \eta_{jk} \mathrm{cos}(\tau_j\omega_k)F^\text{even}(i\omega_k)\hspace{1.5em}{\color{white}.}\label{ct_even_w_to_t}
  \end{align}
\end{minipage}
